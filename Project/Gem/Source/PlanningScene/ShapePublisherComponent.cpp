...
namespace ROSCon2023Demo
{
    void ShapePublisherComponent::Activate()
    {
        ...
        auto topic = m_topicNamespace + m_topicName;
        m_publisher = ros2Node->create_publisher<moveit_msgs::msg::CollisionObject>(topic.c_str(), 10);
    ...
    void ShapePublisherComponent::Reflect(AZ::ReflectContext* context)
    {
        if (auto serializeContext = azrtti_cast<AZ::SerializeContext*>(context))
        {
            serializeContext->Class<ShapePublisherComponent, AZ::Component>()
                ...
                ->Field("TopicNamespace", &ShapePublisherComponent::m_topicNamespace)
                ->Field("TopicName", &ShapePublisherComponent::m_topicName)
                ->Version(1);
            ...
            if (AZ::EditContext* editContext = serializeContext->GetEditContext())
            {
                editContext->Class<ShapePublisherComponent>("ShapePublisherComponent", "Publishes CollisionObject data about shape.")
                    ...
                    ->DataElement(
                        AZ::Edit::UIHandlers::Default,
                        &ShapePublisherComponent::m_topicNamespace,
                        "Topic Name",
                        "Topic namecpace on which CollisionObject messages will be sent.)")
                    ->DataElement(
                        AZ::Edit::UIHandlers::Default,
                        &ShapePublisherComponent::m_topicName,
                        "Topic Name",
                        "Name of topic on which CollisionObject messages will be sent. (usually <namespace>/collision_object)")
                    ;
            }
        }
    }
